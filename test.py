...
data.loc[data['Prime Video'] == 1, 'Prime Video'] = ' Prime Video '
data.loc[data['Prime Video'] == 0, 'Prime Video'] = ''

data.loc[data['Hulu'] == 1, 'Hulu'] = ' Hulu '
data.loc[data['Hulu'] == 0, 'Hulu'] = ''

data.loc[data['Disney+'] == 1, 'Disney+'] = ' Disney+'
data.loc[data['Disney+'] == 0, 'Disney+'] = ''

data["Platform"] = data["Netflix"] + data["Prime Video"] + data["Hulu"] + data["Disney+"]

data = data.drop(['Netflix', 'Prime Video', 'Hulu', 'Disney+'], axis=1)

headerNames = data.columns.values

valuesOfData = []
for header in headerNames:
    valuesOfData.append(data[header])

fig = go.Figure(data=[go.Table(
    header=dict(values=headerNames,
                fill_color='#3B4154',
